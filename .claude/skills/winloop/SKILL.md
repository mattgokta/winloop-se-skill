...
1. An explicit mode request — the word `message`, `prepare`, `debrief`, or `checkpoint`, or equivalent phrasing such as "prep me for", "where does this stand" — always wins, even when the input also contains completed-meeting notes.
...
Every Debrief and Checkpoint response contains these sections, in order — no section omitted, none renamed, none added:
...
Debrief and Checkpoint end with the `Next best action` field; in Prepare, the exact ask and fallback route serve this role, and in Message it is the discovery bridge. In a multi-mode run, close the whole response by restating the single next best action. If the customer is ready, the action is the explicit technical-win ask. If not, the action is the smallest validation step that makes that ask defensible.

## Carry state between runs

WinLoop is a loop, and each invocation is stateless unless the user supplies history. After a Debrief or Checkpoint, offer to append the dated `SE Decision Assist` block to the account's ledger at `opportunities/<account>/<account>.md` — one folder per account; derive `<account>` by lowercasing the account name and replacing spaces and punctuation with hyphens (`Acme Corp` → `opportunities/acme-corp/acme-corp.md`). Generated artifacts for the account (dashboards, customer-facing pages, logos) live in the same folder beside the ledger. Before concluding no ledger exists, list the `opportunities/` directory and match loosely. When a ledger exists for the account under discussion, read it before Checkpoint, Prepare, or a message revision. If the user declines file writes, ask them to paste the previous output under a `Prior state:` header instead.
...
On request, generate account artifacts into the account's `opportunities/<account>/` folder:
...
- **Customer-facing artifacts** (workshop pages, agendas) are built from the ledger's confirmed content, never from the raw debrief. They must contain no accuracy flags, no forecast or confidence data, no Salesforce content, no unconfirmed capability claims, and no internal coaching or commercial language. Write them in the customer's language. Frame paid-scope items honestly but neutrally.
- Make HTML artifacts self-contained: inline CSS and embed images as base64 so files survive email.
- When `templates/customer-workshop.html` exists in this skill's folder, use it as the base for customer-facing pages.
