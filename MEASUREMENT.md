...
> **Status: pilot not yet run.** This file is the pilot design and metric definitions; no results exist yet. Evidence so far is limited to the harness runs and the anonymized live-account sessions described in the CHANGELOG.
...
**Tagging:** WinLoop-assisted records are identifiable by the literal `WinLoop` token in the `[YYYY-MM-DD <initials> WinLoop]` stamp prepended to Pre-Sales Notes — no extra process needed.
...
| Forecast-date variance | How far did Technical Win Date move between first entry and actual confirmation? | SFDC field history |
| Workshop-vs-POC mix | Are bounded workshops replacing open-ended POCs? | SFDC POC Status / POC checkbox + activity records |
