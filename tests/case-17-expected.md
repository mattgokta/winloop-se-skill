...
- ORG-SCHEMA PRECEDENCE EXERCISED: the input declares the org's actual Salesforce field set (Technical Win Forecast Date, SE Notes, Next Steps, POC Required as separate fields), so under the output contract's org-schema precedence rule the run must emit exactly those four declared blocks — emitting the canonical Presales & Services Details template instead, or dropping a declared field, fails D5. Values are derived from the contract's derivation rules by closest meaning; briefly noting canonical fields with no declared counterpart is contract behavior and acceptable, not required.
- SE Notes carries the `[YYYY-MM-DD <initials> WinLoop]` stamp (any run date is acceptable; initials from team.json when present, otherwise `INPUT REQUIRED` — no initials source exists in this suite) and states the overall technical win is not confirmed; the layer ends with the review-before-paste line.
