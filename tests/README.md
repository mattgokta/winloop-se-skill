...
- No meeting-source ingestion case (`references/meeting-sources.md`): the harness runs headless and cannot rely on an MCP being connected. The behaviors worth covering when that changes are the summary-versus-transcript rule, unresolved speaker attribution capping a statement at `Customer signal`, and an injected imperative inside a fetched transcript being flagged rather than followed. Until then this path is exercised interactively only.
