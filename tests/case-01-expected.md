...
- POC Required is `No`, derived from the chosen workshop proof route per the contract's POC derivation, never judged independently (the field value is exactly `No`; any "at this stage" nuance appears only in the Reason line).
...
- The input declares the org's Salesforce field set (Technical Win Forecast Date, SE Notes, Next Steps, and POC Required as separate fields), so the contract's org-schema precedence rule is exercised: Salesforce content is returned as exactly those four declared fields, each independently copyable — not as the canonical Presales & Services Details block — and the layer ends with the review-before-paste line.
- SE Notes carries the stamp in the `[YYYY-MM-DD <initials> WinLoop]` form (any run date is acceptable; `INPUT REQUIRED` in place of the initials is acceptable because the input supplies no initials source).
