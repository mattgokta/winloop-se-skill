...
8. **U8 — Language policy.** The three language slots are respected: SE Decision Assist mirrors the request language, the Salesforce layer is always English regardless of request or source language, and customer-facing artifacts follow the customer's language rather than the SE's. Verbatim quotes stay in their original language throughout (glossed in English inside the Salesforce layer); enumerated values, product names, and the `WinLoop` stamp token are never translated; explicit overrides are honored.
...
14. **D5 — CRM emission and usability.** In Debrief: the Salesforce layer follows the output contract's schema — by default the three banner-grouped **Presales & Services Details** blocks (Presales Information / Presales Notes / Presales Reporting) plus the review-before-paste line, every field independently copyable, with the `[YYYY-MM-DD <initials> WinLoop]` Pre-Sales Notes stamp (initials from team.json, or `INPUT REQUIRED` when no initials source exists). When the case input declares the org's actual field set, the contract's org-schema precedence rule applies instead: exactly the declared fields are emitted, values derived per the derivation rules. Outside Debrief: Salesforce blocks appear only on an explicit CRM-update request.
...
17. **D8 — POC field consistency.** The emitted POC field values are derived from the chosen `Shortest proof route`, never judged independently. The `Not Required` / `TBD` split turns on whether a route is *settled*, not on where it sits on the ladder. Canonical schema: a settled route below POC → `POC Status: Not Required` and `POC` checkbox `No`; POC chosen and bounded → `POC Status: Planned` (or `In Progress`/`Completed` on explicit evidence) and checkbox `Yes`; no route settleable yet — whether the uncertainty is unidentified, or known but not yet resolvable into an agreed route → `POC Status: TBD` (sentinel; picklist left unchanged, nuance recorded in Pre-Sales Notes) and checkbox `No`. Under an org-declared `POC Required` field, the same derivation maps to exactly `Yes`, `No`, or `TBD` with nuance in the `Reason` line. Enum values are exact; explanatory text never lives inside the field value.
