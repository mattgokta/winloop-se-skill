...
17. **D8 — POC field consistency.** The emitted POC field values are derived from the chosen `Shortest proof route`, never judged independently. Canonical schema: a chosen route below POC → `POC Status: Not Required` and `POC` checkbox `No`; POC chosen and bounded → `POC Status: Planned` (or `In Progress`/`Completed` on explicit evidence) and checkbox `Yes`; no route determinable → `POC Status: TBD` (sentinel; picklist left unchanged, nuance recorded in Pre-Sales Notes) and checkbox `No`. Under an org-declared `POC Required` field, the same derivation maps to exactly `Yes`, `No`, or `TBD` with nuance in the `Reason` line. Enum values are exact; explanatory text never lives inside the field value.
