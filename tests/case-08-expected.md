...
- The "wrapped up this quarter" timing is vendor-driven: the forecast window is `Unknown` or explicitly labeled a vendor/internal target, no exact date is invented, the Salesforce `Technical Win Date` field is `INPUT REQUIRED` with the date-picker left-unchanged guidance, and Confidence is `Low` with a rationale naming the open criterion and the unscheduled decision checkpoint — not the SE's quarter-end push (U3, U4).
...
- The input declares no org Salesforce field set, so the org-schema precedence rule is NOT triggered and the canonical **Presales & Services Details** template applies: the three banner groups — Presales Information, Presales Notes, Presales Reporting (checkboxes) — with every contract field present in the contract's order, each value independently copyable, ending with the review-before-paste line; Pre-Sales Notes opens with the `[YYYY-MM-DD <initials> WinLoop]` stamp (initials from `team.json` when present, otherwise `INPUT REQUIRED`; any run date acceptable) (D5).
- Derived Presales Information values: `Pre-Sales confidence for Quarter` is `Low` (equal to the forecast Confidence); `Presales Stage` is `3 - Solution Development` (requirements known — the three criteria agreed at kickoff — with validation in progress against them; `2 - Discovery & Technical Qualification` is acceptable only with an explicit criteria-completeness rationale, and `4 - Validate Solution` fails because status is not `Ready to ask`); `Technical Differentiation` is `Neutral` (no competitive signal in the evidence); `Presales Concern` is `Product` (the delegated-admin capability is the unverified decision-critical gap); `Risks/Gaps` includes `P-Product`, may add `D-Decision Process` (no customer-stated decision timeline), and does NOT include `C-Champion` (the technical decision owner is named) (D5).
- `POC Status` is exactly `Not Required` and the `POC` checkbox is exactly `No` — derived from the focused-demonstration route, never judged independently — with the focused-demo alternative named in Pre-Sales Notes; no explanatory text lives inside either enum value (D8).
- Reporting checkboxes are derived, not defaulted: `RFX` is `No`, `SE Needed` is `Yes`, `Demo Platform Used` is `Yes` (demos are part of the proof route), `Mutual Delivery Plan` is `No` (D5).
- Pre-Sales Notes are decision-focused: they state the overall technical win is not confirmed, record only the scoped SSO and MFA confirmations plus the open delegated-admin criterion, and do not record the hedged affirmation as a confirmation or include internal coaching language (D7).
- Pre-Sales Next Steps carry owners and dates or `TBD`, with customer and SE/vendor actions separated, and include the technical decision checkpoint (D6).
...
INPUT REQUIRED — exact date not established; no customer-stated window (quarter-end is an internal vendor target). (Date-picker field: leave unchanged; note carried in Pre-Sales Notes.)
...
[2026-07-30 INPUT REQUIRED WinLoop] Forecast date INPUT REQUIRED — no scheduled confirmation event; quarter-end timing is an internal target, not customer-stated. Working session 2026-07-30 with the Director of Identity Infrastructure (technical decision owner). Scoped confirmations: the SSO migration approach covers the application inventory ("the migration path works for us"), and he is comfortable with the adaptive MFA policies reviewed last week. Overall technical win is not confirmed: the delegated administration criterion — the subsidiary scenario with scoped role handoff, end to end — has not been demonstrated, and the decision owner stated he will not sign off before seeing it. POC not required: the single open criterion is resolvable by a focused live demonstration of that scenario against a pass condition agreed in advance, followed by a technical decision checkpoint to ask for explicit confirmation against the three agreed criteria. (Prepend above existing notes; do not replace.)
